        ...
        for proverb_id in target_ids:
            try:
                ...
                if not request.force_refresh and proverb_id in analysis_cache:
                    ...
                    # 새로 분석
                    analysis_result = difficulty_analyzer.analyze_proverb_difficulty(proverb_id)
    ...
    try:
        ...
        for result in all_results:
            if result["difficulty_level"] > 0:
                analysis_cache[result["proverb_id"]] = result
        
        processing_time = time.time() - start_time
        server_stats["last_batch_analysis"] = datetime.now().isoformat()
        server_stats["analysis_count"] += len([r for r in all_results if r["difficulty_level"] > 0])
...
async def get_cache_status():
    ...
    try:
        ...
        # 전체 속담 수 조회
        total_proverbs = proverb_database.get_proverb_count() if proverb_database else 0
        
        return {
            "cache_size": len(analysis_cache),
            "total_proverbs": total_proverbs,
            "coverage": f"{(len(analysis_cache) / max(total_proverbs, 1)) * 100:.1f}%",
            "difficulty_distribution": difficulty_distribution,
            "total_score": total_score,
            "average_score": round(total_score / max(len(analysis_cache), 1), 2),
            "last_batch_analysis": server_stats.get("last_batch_analysis"),
            "stats": {
